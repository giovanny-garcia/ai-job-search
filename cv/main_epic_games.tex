...
\vspace{6pt}
\small{Computer Science graduate (Associate of Science, Austin Community College) who builds tools other people can run: a TypeScript Discord bot designed around a 3-request/day API quota, and a Claude Code workflow that turns job-search admin into a repeatable process. Seeking Epic's paid Tools Programmer Intern role on the Build Operations Tools team (May 3 to August 27, 2027) to improve Unreal Editor, UEFN, and Fortnite developer workflows: cheaper human interactions, visible usage data, and less friction in how people write, build, and ship.}
...
\item \textbf{Developer tools and UX:} Ship slash-command tools, embed layouts, and settings flows in discord.js so a small group can browse data, subscribe to alerts, and check quota without opening the vendor site. No public desktop GUI-framework work. Transferable piece is interaction design for tools people click: permission-gated settings, ephemeral status, and operator commands.

\item \textbf{Data processing, caching, and usage reporting:} REST clients, SQLite schemas, upserts, and a cache-first sync so daily use does not burn a tight API budget. A \texttt{/quota} command reports daily API usage and cache status from persisted counters. This is usage reporting from local counters, not a metrics pipeline or dashboard.
...
\item \textbf{Languages:} Production work in TypeScript and JavaScript. Coursework foundation in C++ and C\# (LinkedIn-listed; no public Unreal or C\# game repo). Willing to learn C++ on a live tools problem. C\# game features remain a gap. The shipped work is TypeScript tooling over Counter-Strike 2 match data, not Unreal gameplay code.
...
\begin{itemize}
    ...
    \item Designed the operator UX: \texttt{/subscribe}, \texttt{/settings}, and \texttt{/quota} (ephemeral usage and cache status). Manage Server permission gates sync and settings. Documented the free-tier call budget so another person can run the bot without burning quota on boot. Efficiency bar I already use: fewer manual checks, visible usage, no surprise API spend.
    \item Packaged the project so another person can run it: Node 18+, \texttt{tsx} for development, TypeScript compile to \texttt{dist/}, \texttt{.env.example}, and a README covering setup, environment variables, and a recommended first sync. This is Node/TypeScript packaging, not Unreal cook or package.
\end{itemize}}}
...
Coursework foundation in programming and software development, including LinkedIn-listed C++ and C\#. No public Unreal or C\# game repo. React is LinkedIn-listed only and unused in the projects above.
}}
...
\item \textbf{Practices:} Cache-first API design, quota tracking and usage reporting, slash-command UX, embed reporting, TypeScript packaging to dist/, README-level documentation, agentic workflows with human review.
\item \textbf{Not yet in public work:} Unreal Engine, Unreal Editor, Unreal Editor for Fortnite, C\# game features, desktop GUI frameworks, unit/functional test suites, continuous integration. Honest gaps. I learn unfamiliar tools on a live problem, which is how the Discord bot and Claude Code workflow got built.
\end{itemize}
