...
\vspace{6pt}
\small{Computer Science graduate (Associate of Science, Austin Community College) who builds operating tools, not research demos. I translate a messy user process into typed software: REST APIs, SQLite persistence, and workflow automation so someone else can run the system without me. I use Claude Code for AI-assisted development on implementation, debugging, and structured documents. Seeking TBD Investors' paid, remote Software Engineering Intern role (20 hours a week) to ship internal operating tools, integrations, and practical AI across founder-led portfolio companies.}
...
\item \textbf{AI-enabled solutions:} Uses Claude Code for practical, AI-assisted development: evaluate messy problems, draft structured documents, and multiply output on implementation and debugging while keeping the architecture decisions.

\item \textbf{Independent remote delivery:} Owns a project from requirement to documented code. README-level setup, environment-variable contracts that keep secrets out of source, and a usage strategy so a nontechnical operator can run the tool.
...
\begin{itemize}
    ...
    \item Modeled the database around real operating needs: per-guild settings so one codebase can serve multiple groups, seen-item deduplication, cached match/results/country datasets, and daily API usage counters (WAL mode, upserts).
    \item Tested and improved reliability in the poll loop: exponential backoff when a pass fails, typed API errors, and a \texttt{/quota} command so an operator can see remaining calls and cache status without opening the vendor site.
    \item Wrote maintainable, documented code: setup, environment variables, and a free-tier usage plan so another person can run the bot without burning the daily quota on boot.
\end{itemize}}}
...
Coursework foundation in programming and software-development fundamentals. LinkedIn-listed skills include C++, C\#, and React.
}}
...
\item \textbf{Practices:} quota-aware API clients, caching, structured errors, README-driven setup, iterating from real user constraints, secrets in environment variables rather than source.
\item \textbf{Not yet in public work:} production web applications, cloud platforms, and data pipelines. I learn unfamiliar tools on a live problem, which is how the Discord bot and Claude Code workflow got built.
\end{itemize}
...
\vspace{1pt}
\begin{itemize}
\item \textbf{How I work:} I pick a user problem, constrain the design (quotas, permissions, reminders), then ship a vertical slice and document it. Ready to sit with a portfolio-company operator, turn the manual process into requirements, and ship without waiting for a ticket queue.
\item \textbf{Work setup:} Austin, Texas. US-based. English. Available 20 hours a week for the remote internship, or a fuller summer stretch if that helps a portfolio company.
\end{itemize}
